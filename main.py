...
class Bot(BaseBot):
    ...
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        ...
        await self.highrise.tg.create_task(self.highrise.teleport(
            session_metadata.user_id, Position(15.00, 0.00, 19.00, "FrontRight")))
             

    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        await self.highrise.chat(f"Bienvenido Rockero! @{user.username} Disfruta del mejor Rock!")
        try:
            emote_name = random.choice(list(secili_emote.keys()))
            emote_info = secili_emote[emote_name]
            emote_to_send = emote_info["value"]
            await self.highrise.send_emote("dance-metal",)
    ...
    async def on_user_leave(self, user: User):
        ...
        user_id = user.id
        farewell_message = f"Nos Vemos @{user.username}"
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith("beso"):
        ...
        if                          message.lower().startswith("git") or message.lower().startswith("tp"):
          target_username =         message.split("@")[-1].strip()
        ...
        if message.lower().startswith("--") and await self.is_user_allowed(user):
            ...
            if len(parts) == 2 and parts[1].startswith("@"):
                ...
                if target_user:
                    ...
                    print(f"Nombre '{target_username}' no se encuentra.")
        ...
        if message.lower() == "follow" and await self.is_user_allowed(user):
            if self.following_user is not None:
                await self.highrise.chat("Actualmente estoy siguiendo a alguien más")
            else:
                await self.follow(user)

        if message.lower() == "stop" and await self.is_user_allowed(user):
            if self.following_user is not None:
                await self.highrise.chat("Dejé de seguir")
                self.following_user = None
            else:
                await self.highrise.chat("No estoy siguiendo a nadie.")
        ...
        if message == "stop" or message == "dur":
            if user_id in self.user_emote_loops:
                if self.user_emote_loops[user_id] == "dans":
...
#Ulti Ulti Ulti Ulti Ulti Ulti Ulti
    ...
    async def start_random_emote_loop(self, user_id: str) -> None:
        self.user_emote_loops[user_id] = "emote"
  ...
  #Genel Genel Genel Genel Genel
    ...
    async def is_user_allowed(self, user: User) -> bool:
        ...
        return user_privileges.moderator or user.username in ["user3","user2","user1"]
...
#gellllbbb
    ...
    async def userinfo(self, user: User, target_username: str) -> None:
        ...
        if not user_info.users:
            await self.highrise.chat("texto2")
            return
        ...
        await self.highrise.chat(f"""Nombre: {target_username}\nSeguidores: {number_of_followers}\nAmigos: {number_of_friends}\nNacimiento: {joined_at}\nDias en el juego: {days_played}""")
...
class RunBot():
  room_id = "667f579ba58ccd4dda1d7b64"
  bot_token = "eba61d64d9751ea1a7d5cc3437d37a6629c83940473dde5b131f4923be7617d8"
  bot_file = "main"
  bot_class = "Bot"
